  ...
  return (
    <div className="bg-slate-900/90 rounded-xl border border-slate-800 p-4 sm:p-5 shadow-sm flex flex-col justify-between h-full">
      ...
      <div className="flex items-center justify-between pb-3 border-b border-slate-800 mb-4">
        <div className="flex items-center space-x-2">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-400">MPU6050 IMU + Laser Line & RPi Camera</p>
          </div>
        </div>

        {/* Fusion Status Badge */}
        <div className={`px-3 py-1 rounded-lg border text-xs font-mono font-semibold flex items-center space-x-1.5 ${badgeColor}`}>
          {confidence >= 66 ? (
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center my-1">
        ...
        <div className="md:col-span-5 flex flex-col items-center justify-center p-3 bg-slate-950/60 rounded-xl border border-slate-800/80">
          <div className="relative w-32 h-32 flex items-center justify-center">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
              <span className="text-2xl font-bold font-mono tracking-tight text-white">
                {confidence}<span className="text-sm text-slate-400">%</span>
              </span>
              <span className="text-[9px] font-mono uppercase text-slate-400 font-medium tracking-wider">
                CONFIDENCE
              </span>
            </div>
          </div>
          ...
          <p className="text-[10px] font-mono text-slate-400 mt-2 text-center">
            Location: <strong className="text-amber-400">{currentTelemetry.position.toFixed(2)} m</strong>
          </p>
        </div>
      ...
      <div className="mt-3 pt-2 border-t border-slate-800/80 flex flex-wrap items-center justify-between text-[11px] font-mono text-slate-400">
        <span className="text-slate-500">
          RPi 4 Rule: <strong className="text-slate-300">MPU6050 Jolt (&gt;1.5G) + RPi Cam Gauge Spread (&gt;8mm) = CRITICAL</strong>
        </span>
